...
prediction <- function(inword){
  ...
  if(word_length >= 3){
    ...
    if(nrow(prediction_list) >= 1){
      return(as.character(prediction_list$word4[1]))
    }
  }
    ...
    prediction_list <- prediction_list[order(-prediction_list$Frequency), ]
    if(nrow(prediction_list) >= 1){
      return(as.character(prediction_list$word3[1]))
    }
  }
    ...
    prediction_list <- prediction_list[order(-prediction_list$Frequency), ]
    if(nrow(prediction_list) >= 1){
      return(as.character(prediction_list$word2[1]))
    }
  }
}
